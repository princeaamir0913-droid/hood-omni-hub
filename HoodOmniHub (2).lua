...
local function StartEngineLoops()
    ...
    RunService.Heartbeat:Connect(function(dt)
        ...
        if espTimer >= 0.5 then
            ...
            if Config.ESP.GunESP then
                for _, v in ipairs(workspace:GetDescendants()) do
                    if v:IsA("Tool") and not v:FindFirstChild("_GunGlow") then
        ...
        -- Loot/Pickup (every 0.3s)
        lootTimer += dt
        if lootTimer >= 0.3 then
            lootTimer = 0
            local hrp = Util.getHRP()
            ...
            if Config.Loot.AutoPickup then
                for _, v in ipairs(workspace:GetDescendants()) do
                    if v:IsA("Tool") and v.Parent ~= LocalPlayer.Character and v.Parent ~= LocalPlayer.Backpack then
            ...
            -- Auto Loot (proximity prompts with cash/money keywords)
                ...
                for _, v in ipairs(workspace:GetDescendants()) do
                    if v:IsA("ProximityPrompt") then
                        ...
                        if name:find("cash") or name:find("money") or name:find("loot")
                            or action:find("pick") or action:find("loot") or action:find("collect") then
...
function loadGangWars(UI, Config, Util, safeCall, AntiDetect)
    ...
    UI.Tab("🔫", "Weapons", function(p)
        ...
        local weapons = {
            ...
            "Revolver", "Choppa", "SMG",
            -- Gamepass Glock Variants
            "Glock19X", "Glock19", "Glock26 Switch", "Glock Gold Switch",
            -- Gamepass P80 Variants
            "P80 Switch", "P80",
            -- Gamepass AR9
            "AR9 Fully",
            -- Gamepass 150R/200R Easter
            ...
            -- Gamepass Mini Gun
            "Mini Gun",
            -- Gamepass Uzi Variants
            "Mr Beast Uzi", "Mr Beast Fully",
            -- Gamepass Special Switches
